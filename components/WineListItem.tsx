...
export const InventoryItemCard: React.FC<ItemProps> = ({ item, onUpdateStock, onRemove }) => {
  ...
  if (isGrape(item)) {
    ...
    if (item.minStock && quantityValue <= item.minStock) {
        badges.push({ label: '⚠️ Stock Bajo', color: 'bg-red-900 text-red-200' });
    }
  }
  ...
  return (
    <div className="bg-slate-800 rounded-xl shadow-lg p-5 flex flex-col transition-all duration-300 hover:shadow-indigo-500/10 hover:ring-1 hover:ring-slate-600 relative overflow-hidden">
